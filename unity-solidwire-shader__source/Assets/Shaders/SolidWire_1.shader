    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void geom(triangleadj v2g IN[6], inout TriangleStream<g2f> OUT)
            {
                ...
                float2 _p0 = IN[0].pos.xy;
                float2 _p1 = IN[1].pos.xy;
                float2 _p2 = IN[2].pos.xy;

                float4 p0 = float4(_p0, 0, IN[0].pos.w);
                float4 p1 = float4(_p1, 0, IN[1].pos.w);
                float4 p2 = float4(_p2, 0, IN[2].pos.w);
                ...
                if (isEdgeDrawn(adj.x, IN[1].idxType.y)){
                    appendEdge(p0, p1, IN[1].color, edge0Length, false, OUT);
                }

                if (isEdgeDrawn(adj.y, IN[2].idxType.y)){
                    appendEdge(p1, p2, IN[2].color, edge1Length, false, OUT);
                }

                if (isEdgeDrawn(adj.z, IN[0].idxType.y)){
                    appendEdge(p2, p0, IN[0].color, edge2Length, false, OUT);
                }
            }
